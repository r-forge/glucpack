minmod4<-function(times,conc,i,starta,id){
    require(odesolve)
    require(minpack.lm)
                                        #assumed contants
    V=1.7
...
###################################################################################
	...
	with(as.list(parameters),{
            dy1=-(Sg+Y[2])*Y[1]+Sg*Gb+ra((a1)^2,(a2^2),(a3^2),t)/V
...
#################################################################################
    ...
    trial=nls.lm(par=c(p2 = sqrt(.03093), p3 = sqrt(.02062),a1=starta[1],a2=starta[2],a3=starta[3]),fn=fit2)
    ...
    ras=sapply(0:120,function(x){ra(trial$par[3],trial$par[4],trial$par[5],x)})
