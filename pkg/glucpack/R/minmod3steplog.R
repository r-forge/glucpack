minmod3log<-function(times,conc,i,starta){
    require(odesolve)
    require(minpack.lm)
                                        #assumed contants
    V=1.7
...
###################################################################################
	...
	with(as.list(parameters),{
            dy1=-(Sg+Y[2])*Y[1]+Sg*Gb+ra(exp(a1),exp(a2),t)/V
            dy2=(-exp(p2))*Y[2]+exp(p3)*(Insulin(t)-Ib)
            list(c(dy1, dy2))
	})
    }
...
#################################################################################
    ...
    trial=nls.lm(par=c(p2 = log(.03093), p3 = log(.02062),a1=starta[1],a2=starta[2]),fn=fit2)
    test=lsoda(c(Gb,0), 0:120, model2, trial[1]$par)
    par(mfrow=c(3,1))

    plot(0:120,test[,2],type="l",main="Glucose",xlab="min",ylab="glucose")
    points(times,conc)
    si=1.7*(exp(trial$par[2])/exp(trial$par[1]))
